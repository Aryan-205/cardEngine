...
export default function EditorPage() {
  const [activeTab, setActiveTab] = useState<'text' | 'bg' | 'envelope'>('text');
  ...
  return (
    <div className="flex h-screen bg-[#E5E7EB] font-sans">
      ...
      <main className="flex-1 relative flex items-center justify-center p-12">
        { activeTab === 'envelope' && <EnvelopePreview envelopeColor={envelopeSettings.color} linerImage={envelopeSettings.liner} />}
      ...
      <aside className="w-[400px] bg-white border-l overflow-y-auto p-6 shadow-2xl">
        ...
        {activeTab === 'envelope' && <EnvelopeControls 
          settings={envelopeSettings} 
          updateSettings={(newSettings: any) => setEnvelopeSettings(prev => ({ ...prev, ...newSettings }))} 
        />}
        {activeTab !== 'text' && activeTab !== 'bg' && activeTab !== 'envelope' && (
