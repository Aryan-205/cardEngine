...
import { GlassInputWrapper } from '@/component/glass-input-wrapper';
import { useImageStore } from '@/lib/store';
import {  Trash2, Eye, EyeOff } from 'lucide-react';
import { fontFamilies, getAvailableFontWeights } from '@/lib/fonts';
...
export const TextOverlayControls = () => {
  ...
  const selectedOverlay = textOverlays.find(
    (overlay: TextOverlay) => overlay.id === selectedOverlayId
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-sm text-white">Text Overlays</h3>
        <Button
          variant="outline"
          size="sm"
          onClick={clearTextOverlays}
          disabled={textOverlays.length === 0}
          className="h-8 px-3 text-xs font-medium rounded-lg text-black"
        >
          Clear All
        </Button>
      </div>
          ...
          <div className="space-y-2 max-h-32 overflow-y-auto">
            {textOverlays.map((overlay) => (
              ...
              >
                ...
                <span className="flex-1 text-xs text-black truncate">{overlay.text}</span>
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={(e) => {
